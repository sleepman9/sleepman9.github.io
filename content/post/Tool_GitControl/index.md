...
## 学习网站
[Learn Git Branching模拟沙盒](https://learngitbranching.js.org/?locale=zh_CN)
...
2. ​`git merge`​合并到当前分支（如果父节点除了当前分支还有一个，不需要带参数，如果有多个需要指明合并哪一个）
...
## QA
1. `push`失败。一般是因为当前同名分支有新代码冲突，需要先更新当前分支，直接`push`自己命名的分支，可以避免此类。
